...
function pickMenu(category: SkillCategory, currentLevel: number) {
  ...
  // 最小レベル <= 現在のレベル <= 最大レベル に該当するメニューを探す
...
export default function PlayerPage() {
  ...
  const submitDailyReport = async () => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/practice-logs`, {
        method: "POST",
    ...
    <main className="min-h-screen bg-[#f0f9ff] font-['M_PLUS_Rounded_1c'] text-slate-800 pb-32">
      ...
      <div className="mx-auto w-full max-w-3xl px-4 py-6 space-y-6">
        ...
        <div className="rounded-[32px] bg-gradient-to-br from-blue-600 to-blue-400 p-6 text-white shadow-xl shadow-blue-200">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-black">きょうの選手データ</h1>
              <p className="opacity-90 text-sm font-bold">めざせ！未来のプロ野球せんしゅ！</p>
            </div>
            <div className="bg-white/20 backdrop-blur-md rounded-2xl p-3 text-center border border-white/30">
              <div className="text-[10px] font-bold">PLAYER ID</div>
              <div className="text-xl font-black">{playerId}</div>
            </div>
          </div>
        </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 px-2">
            ...
            <h2 className="text-lg font-black text-blue-900">いまのキミの実力</h2>
          </div>
                ...
                <Card className="rounded-[24px] border-none shadow-md bg-white overflow-hidden">
                  ...
                  <div className="h-1.5 w-full bg-slate-100">
                    <div 
                      ...
                      style={{ width: `${(11 - skillLevels[cat]) * 10}%` }}
                    />
                  </div>
                </Card>
              </div>
              ...
              return (
                <div key={cat} className="p-4 rounded-2xl bg-orange-50 border-2 border-orange-100">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-black text-orange-700">{SKILL_LABEL[cat]}</span>
                    <Badge className="bg-orange-500 text-white border-none">{skillLevels[cat]}級メニュー</Badge>
                  </div>
                  <div className="text-sm font-bold text-slate-700 bg-white p-3 rounded-xl shadow-sm">
                    {menu.title}
                    <ul className="mt-2 text-xs text-slate-500 space-y-1 list-none">
                      {menu.items.map((it, i) => <li key={i}>✅ {it}</li>)}
                    </ul>
                  </div>
                </div>
              );
        ...
        <section ref={reportRef} className="space-y-4 pt-4">
          <div className="flex items-center gap-2 px-2">
            <span className="text-xl">📝</span>
            <h2 className="text-lg font-black text-blue-900">れんしゅう日報</h2>
          </div>
          ...
          <Card className="rounded-[32px] border-none shadow-lg bg-white p-6 space-y-6">
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {SKILL_KEYS.map((cat) => (
                  <button
                    key={cat}
                    onClick={() => setTags(prev => prev.includes(cat) ? prev.filter(t => t !== cat) : [...prev, cat])}
                    className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${
                      tags.includes(cat) ? "bg-blue-600 text-white shadow-md scale-105" : "bg-slate-100 text-slate-400 hover:bg-slate-200"
                    }`}
                  >
                    {SKILL_LABEL[cat].split(" ")[0]} {SKILL_LABEL[cat].split(" ")[1]}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-black text-slate-600 flex justify-between">
                  <span>😊 きぶん</span>
                  <span className="text-blue-500">{mood}</span>
                </label>
                <input type="range" min="1" max="5" value={mood} onChange={e => setMood(Number(e.target.value))} className="w-full accent-blue-500" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-black text-slate-600 flex justify-between">
                  <span>💪 つかれ</span>
                  <span className="text-blue-500">{fatigue}</span>
                </label>
                <input type="range" min="1" max="5" value={fatigue} onChange={e => setFatigue(Number(e.target.value))} className="w-full accent-blue-500" />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-black text-slate-600">ふりかえりメモ</label>
              <textarea
                value={body}
                onChange={(e) => setBody(e.target.value)}
                placeholder="ヒットが打てた！次は守備をがんばりたい！"
                className="w-full min-h-[120px] bg-slate-50 border-2 border-slate-100 rounded-[24px] p-4 text-sm focus:border-blue-300 focus:ring-0 outline-none transition-all"
              />
            </div>

            <Button 
              className="w-full h-14 rounded-full bg-green-500 hover:bg-green-600 text-white text-lg font-black shadow-lg shadow-green-100 transition-transform active:scale-95" 
      ...
      <div className="fixed inset-x-0 bottom-6 z-40 px-4">
        <button 
          ...
          className="mx-auto max-w-3xl w-full h-14 rounded-full bg-blue-600 text-white font-black shadow-2xl flex items-center justify-center gap-2 hover:bg-blue-700 transition-all active:scale-95"
        >
          <span>✍️</span> 日報を書く
        </button>
      </div>
