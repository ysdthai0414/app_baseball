...
export default function ParentPage() {
  ...
  const fetchChildDetails = async (player: any): Promise<Child> => {
    try {
      ...
      return { 
        ...
        id: String(player.id), 
        has_evaluation: true, 
        hitting_rank: v.batting||10, throwing_rank: v.throwing||10, catching_rank: v.catching||10, running_rank: v.running||10, iq_rank: v.iq||10 
      };
    } catch {
      return { 
        ...player, 
        ...
        hitting_rank:10, throwing_rank:10, catching_rank:10, running_rank:10, iq_rank:10 
      };
    }
  };
  ...
  const handleRegisterOrSearch = async () => {
    ...
    try {
      ...
      if (!isExisting) {
        // ★修正ポイント：入力されたIDがあればそれを数値にして使う。なければ現在時刻を使う。
        let idToSend: number;
        if (inputProfile.id) {
            idToSend = Number(inputProfile.id);
            if (isNaN(idToSend)) {
                throw new Error("IDは半角数字で入力してください");
            }
        } else {
            idToSend = Date.now();
        }
        ...
        if (resCreate.ok) {
           ...
           targetChild = {
               id: String(resJson.id), // レスポンスのIDを使用
               name: inputProfile.name,
               grade: inputProfile.grade,
               has_evaluation: false,
               hitting_rank:10, throwing_rank:10, catching_rank:10, running_rank:10, iq_rank:10
           ...
           await fetchChildren(); 
            ...
            // "already_exists" の場合は既存データとして扱う
            if (err.status === "already_exists") {
                 alert("そのIDの選手は既に登録されています。");
                 // 必要であればここで詳細取得に飛ばす処理も可能
            } else {
                 throw new Error("登録に失敗しました: " + (err.detail || resCreate.statusText));
            }
        }
      }
    ...
    <main style={styles.container}>
      ...
      {view === "list" && (
        <div style={styles.fadeIn}>
          ...
          <div style={styles.sectionHeader}>
            ...
            <button onClick={() => setView("register")} style={styles.addButton}>＋ 追加 / 検索</button>
          </div>
      ...
      {view === "register" && (
        <div style={styles.fadeIn}>
          ...
          <p style={{fontSize:12, color:"#666", marginBottom:20}}>すでにIDをお持ちの場合は入力してください。<br/>なければ自動発行されます。</p>
          
          <div style={styles.formCard}>
            <label style={styles.label}>ID <span style={{fontSize:10, fontWeight:"normal"}}>(任意・数字のみ)</span>
                <input type="text" placeholder="例: 100" value={inputProfile.id} onChange={e=>setInputProfile({...inputProfile,id:e.target.value})} style={styles.input}/>
            </label>
            <label style={styles.label}>名前 <span style={{color:"red"}}>*</span>
                <input type="text" placeholder="例: たろう" value={inputProfile.name} onChange={e=>setInputProfile({...inputProfile,name:e.target.value})} style={styles.input}/>
            </label>
            <label style={styles.label}>学年 <span style={{color:"red"}}>*</span>
                <select value={inputProfile.grade} onChange={e=>setInputProfile({...inputProfile,grade:e.target.value})} style={styles.input}>
                  <option value="">選択</option>{["小1","小2","小3","小4","小5","小6"].map(g=><option key={g} value={g}>{g}</option>)}
                </select>
            </label>
            
            <div style={{display:"flex",gap:12,marginTop:20}}>
              <button onClick={()=>setView("list")} style={styles.secondaryButton} disabled={loading}>キャンセル</button>
              <button 
          ...
          <div style={styles.detailCard}>
            <div style={styles.detailHero}>
              ...
              <div style={{color:"#64748b"}}>
                {selectedChild.grade} 
                <span style={{fontSize:12, marginLeft:8, background:"#eee", padding:"2px 6px", borderRadius:4}}>ID: {selectedChild.id}</span>
              </div>
            </div>
            {!selectedChild.has_evaluation ? (
              <div style={{padding:40, textAlign:"center", color:"#64748b"}}>
                ...
                <p style={{fontSize:12, marginTop:8}}>
                    監督からの評価を待つか、<br/>
                    ID「<span style={{fontWeight:"bold", color:"#2563eb"}}>{selectedChild.id}</span>」を<br/>
                    監督に伝えてください。
                </p>
